...
from src.models.data_classes import Colors, Dimensions, UIConfig
...
class DateNavigationBar(tk.Frame):
    ...
    def _setup_ui(self):
        ...
        nav_button_style = {**button_style, "width": 1, "padx": 2, "pady": 4}

        # Today button style
        today_button_style = {**button_style, "padx": 8, "pady": 4}

        # Previous day button
        self.prev_button = tk.Button(
            self, text="◀", command=lambda: self._change_date(-1), **nav_button_style
        )
        ...
        self.today_button = tk.Button(
            self, text="Today", command=self._go_to_today, **today_button_style
        )
...
class CalendarDialog:
    ...
    def _setup_ui(self):
        ...
        ttk.Button(nav_frame, text="◀", width=3, command=self._prev_month).pack(
            side="left"
        )
        self.month_year_label = ttk.Label(
            nav_frame, text="", style="CalendarHeader.TLabel"
        )
        self.month_year_label.pack(side="left", expand=True, padx=10)
        ttk.Button(nav_frame, text="▶", width=3, command=self._next_month).pack(
            side="right"
        )
        ...
        for i, day in enumerate(weekdays):
            ttk.Label(self.cal_frame, text=day, style="Calendar.TLabel").grid(
